---
title: "Automate Downloads with the Input Buffer"
...
| `all` | Select every non-storefront item in the current menu for download. The client expands this into a sequence of numeric indices followed by `n` (confirm) entries and injects them into the front of the buffer. |
...
When `all` is reached, the client inspects the current menu, builds the full list of downloadable-item indices interleaved with `n` confirmations, and injects them into the buffer — so every file downloads in sequence with no further input needed.
